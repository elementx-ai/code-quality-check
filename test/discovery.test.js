...
test("discoverProjects finds Node and Python project roots", async () => {
  const tempDirectory = await fs.mkdtemp(
    path.join(os.tmpdir(), "project-checks-"),
  );
  ...
  try {
    ...
    await fs.mkdir(path.join(tempDirectory, "packages", "web"), {
      recursive: true,
    });
    await fs.writeFile(
    ...
    await fs.mkdir(path.join(tempDirectory, "services", "api"), {
      recursive: true,
    });
    await fs.writeFile(
    ...
    const { projects } = await discoverProjects(tempDirectory, {
      includeRoot: true,
    });
    ...
    assert.equal(
      projects.find((project) => project.relativePath === "services/api")
        .targets[0].metadata.hasRuff,
      true,
    );
  } finally {
...
test("discoverProjects respects projectDepth", async () => {
  const tempDirectory = await fs.mkdtemp(
    path.join(os.tmpdir(), "project-checks-depth-"),
  );
  ...
  try {
    ...
    const { projects: rootOnlyProjects } = await discoverProjects(
      tempDirectory,
      {
        includeRoot: true,
        projectDepth: 0,
    ...
    const { projects: oneLevelProjects } = await discoverProjects(
      tempDirectory,
      {
        includeRoot: true,
        projectDepth: 1,
    ...
    const { projects: unlimitedProjects } = await discoverProjects(
      tempDirectory,
      {
        includeRoot: true,
        projectDepth: -1,
...
test("discoverProjects finds Terraform projects in tf directories", async () => {
  const tempDirectory = await fs.mkdtemp(
    path.join(os.tmpdir(), "project-checks-tf-"),
  );
...
test("discoverProjects finds Terraform projects in monorepo tf directories", async () => {
  const tempDirectory = await fs.mkdtemp(
    path.join(os.tmpdir(), "project-checks-tf-mono-"),
  );
  ...
  try {
    ...
    await fs.mkdir(path.join(tempDirectory, "services", "api", "tf"), {
      recursive: true,
    });
    await fs.writeFile(
...
test("discoverProjects ignores tf directories without .tf files", async () => {
  const tempDirectory = await fs.mkdtemp(
    path.join(os.tmpdir(), "project-checks-tf-empty-"),
  );

  try {
    await fs.mkdir(path.join(tempDirectory, "tf"), { recursive: true });
    await fs.writeFile(
      path.join(tempDirectory, "tf", "readme.md"),
      "# Not terraform\n",
    );
...
test("discoverProjects finds Terraform projects in module directories", async () => {
  const tempDirectory = await fs.mkdtemp(
    path.join(os.tmpdir(), "project-checks-tf-module-"),
  );
...
test("discoverProjects does not flag .tf files inside module directories as misplaced", async () => {
  const tempDirectory = await fs.mkdtemp(
    path.join(os.tmpdir(), "project-checks-module-ok-"),
  );
  ...
  try {
    ...
    const { misplacedTerraformFiles } = await discoverProjects(tempDirectory, {
      includeRoot: true,
    });
...
test("discoverProjects discovers root-level .tf files as a Terraform project", async () => {
  const tempDirectory = await fs.mkdtemp(
    path.join(os.tmpdir(), "project-checks-root-tf-"),
  );
...
test("discoverProjects detects .tf files in non-tf subdirectories", async () => {
  const tempDirectory = await fs.mkdtemp(
    path.join(os.tmpdir(), "project-checks-misplaced-sub-"),
  );
  ...
  try {
    ...
    await fs.mkdir(path.join(tempDirectory, "services", "api", "deploy"), {
      recursive: true,
    });
    await fs.writeFile(
...
test("discoverProjects reports no misplaced files when .tf files are in tf directories", async () => {
  const tempDirectory = await fs.mkdtemp(
    path.join(os.tmpdir(), "project-checks-tf-ok-"),
  );
...
test("discoverProjects reports no misplaced files when no .tf files exist", async () => {
  const tempDirectory = await fs.mkdtemp(
    path.join(os.tmpdir(), "project-checks-no-tf-"),
  );

  try {
    await fs.writeFile(
      path.join(tempDirectory, "index.ts"),
      "export const x = 1;\n",
    );
...
test("discoverProjects finds Terraform projects with .tf files in nested subdirectories of module", async () => {
  const tempDirectory = await fs.mkdtemp(
    path.join(os.tmpdir(), "project-checks-tf-nested-"),
  );

  try {
    await fs.mkdir(path.join(tempDirectory, "module", "vpc"), {
      recursive: true,
    });
    await fs.writeFile(
...
test("discoverProjects finds Terraform projects with .tf files in nested subdirectories of tf", async () => {
  const tempDirectory = await fs.mkdtemp(
    path.join(os.tmpdir(), "project-checks-tf-nested2-"),
  );

  try {
    await fs.mkdir(path.join(tempDirectory, "tf", "modules", "s3"), {
      recursive: true,
    });
    await fs.writeFile(
...
test("discoverProjects finds Terraform projects in root-level tf directories when includeRoot is false", async () => {
  const tempDirectory = await fs.mkdtemp(
    path.join(os.tmpdir(), "project-checks-tf-root-"),
  );
